...
# -*- coding: utf-8 -*-
"""
Casino Ruta 5.1 — Telegram-бот (виртуальные фишки)
...
ADMIN_ID = 718853742        # <-- замени на свой ID администратора
...
logger = logging.getLogger("casino_ruta5_1")

# ---------- BOT ----------
if not TOKEN or TOKEN == "ВАШ_ТОКЕН_ЗДЕСЬ":
    print("ERROR: вставь токен в переменную TOKEN в файле casino_ruta5_1.py")
    exit(1)
...
def format_top(n=10) -> str:
    ...
    for i, (uid, bal) in enumerate(items[:n], start=1):
        name = data["users"].get(str(uid), {}).get("name") or ""
        if name:
...
def cmd_bonus(m: types.Message):
    ...
    if now - last < BONUS_SECONDS:
        ...
        bot.send_message(m.chat.id, f"⏳ Бонус уже взят. Через ~{rem} ч.", reply_markup=main_keyboard())
        return
...
# ---------- TRANSFER (interactive or /transfer id amount) ----------
...
def on_slot_bet(call: types.CallbackQuery):
    ...
    try:
        ...
        bot.send_message(call.message.chat.id, "Ошибка при игре в слоты.")
...
def cmd_bet(m: types.Message):
    ...
    if game in ("slots","slot"):
        ...
        bot.send_message(m.chat.id, "Неизвестная игра. Доступные: slots, roul, dice, guess", reply_markup=main_keyboard())

# ---------- SIMPLE REPLY-BUTTON HANDLERS ----------
...
def btn_help(m: types.Message):
    text = (
        "ℹ️ <b>Помощь</b>\n\n"
        "🎰 <b>Слоты</b> — три барабана. Кнопки 100/500/1000 или /bet slots <сумма>.\n"
        "🎯 <b>Рулетка</b> — шанс ~30%, выигрыш ×3. Кнопки или /bet roul <сумма>.\n"
        "🎲 <b>Кости</b> — просто бросок для фана или /bet dice <сумма> (правило: выигрыш при 4–6 ×2).\n"
        "🎁 <b>Бонус</b> — ежедневные +1000 фишек (/bonus).\n"
        "💸 <b>Перевести</b> — /transfer <id> <amount> или нажми и следуй инструкциям.\n"
