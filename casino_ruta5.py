...
# -*- coding: utf-8 -*-
"""
Casino Ruta 5.0 — полный Telegram-бот-казино (pyTelegramBotAPI)
Поддерживает: слоты (fixed buttons + /bet), рулетка, кости, угадайку, перевод между игроками,
ежедневный бонус, топ, админ-панель. Работает в ЛС и в группах.
Перед запуском:
...
ADMIN_ID = 718853742        # <-- твой id администратора (замени)

DATA_FILE = "casino_data_v5.json"
BACKUP_FILE = "casino_data_v5_backup.json"
...
logger = logging.getLogger("casino_ruta5")

# ---------- BOT ----------
if not TOKEN or TOKEN == "ВАШ_ТОКЕН_ЗДЕСЬ":
    print("ERROR: вставь токен в переменную TOKEN в файле casino_ruta5.py")
    exit(1)
...
def cmd_start(m: types.Message):
    ...
    bot.send_message(m.chat.id, f"🎰 <b>Казино Рута 5.0</b>\nПривет, {name}!\n💰 Баланс: {get_balance(uid)} фишек", reply_markup=kb, parse_mode="HTML")
...
def show_slots_kb(m: types.Message):
    ...
    bot.send_message(m.chat.id, "Выберите ставку для слотов (или используй команду /bet slots <сумма>):", reply_markup=kb)
...
def show_roulette_kb(m: types.Message):
    ...
    bot.send_message(m.chat.id, "Выберите ставку для рулетки (шанс ~30%, ×3) или /bet roul <сумма>:", reply_markup=kb)
...
def cmd_dice(m: types.Message):
    ...
    if not ok:
        bot.send_message(m.chat.id, reason); return
    # Here we allow either fixed or custom bet via /bet dice <amount>
...
# ---------- GUESS NUMBER (fixed stake example) ----------
@bot.message_handler(commands=["guess"])
def start_guess_cmd(m: types.Message):
    # To start: /guess (uses fixed stake 500)
    uid = m.from_user.id
    ensure_user(uid)
    ok, reason = can_play(uid)
    if not ok:
    ...
    msg = bot.send_message(m.chat.id, "Введите число от 1 до 5 (ставка 500):")
    bot.register_next_step_handler(msg, finish_guess_step)

def finish_guess_step(m: types.Message):
    uid = m.from_user.id
    ensure_user(uid)
    if not m.text or not m.text.isdigit():
...
# ---------- COMMAND-BASED BETTING ----------
# Usage: /bet <game> <amount>
# games: slots|slot, roul|roulette|roulette, dice|dice, guess (guess is fixed)
@bot.message_handler(commands=["bet"])
def cmd_bet(m: types.Message):
    parts = m.text.split()
    if len(parts) < 3:
        bot.send_message(m.chat.id, "Использование: /bet <game> <amount>\nПримеры: /bet slots 500  или  /bet roul 1000", reply_markup=main_keyboard())
        return
    game = parts[1].lower()
    if not parts[2].lstrip('-').isdigit():
        bot.send_message(m.chat.id, "Сумма должна быть числом.", reply_markup=main_keyboard()); return
    amount = int(parts[2])
    uid = m.from_user.id
    ensure_user(uid)
    ok, reason = can_play(uid)
    if not ok:
        bot.send_message(m.chat.id, reason); return
    if amount < MIN_BET or amount > MAX_BET:
        bot.send_message(m.chat.id, f"Ставка от {MIN_BET} до {MAX_BET}", reply_markup=main_keyboard()); return
    if get_balance(uid) < amount:
        bot.send_message(m.chat.id, "Недостаточно фишек", reply_markup=main_keyboard()); return

    # route to game
    if game in ("slots","slot"):
        # run slots without animation edit (simple)
        try:
            spin_slots(m.chat.id, uid, amount)
        except Exception as e:
            logger.exception("slots error: %s", e)
            bot.send_message(m.chat.id, "Ошибка при игре в слоты.")
    elif game in ("roul","roulette","roulett","rou"):
        # roulette logic (same as callback)
        change_balance(uid, -amount)
        u = get_user(uid)
        u["games_played"] = u.get("games_played",0)+1
        if random.random() < ROULETTE_CHANCE:
            ...
            change_balance(uid, win)
            u["wins"] += 1
            bot.send_message(m.chat.id, f"🎉 Победа! +{win} фишек\n💰 Баланс: {get_balance(uid)}", reply_markup=main_keyboard())
        else:
            u["losses"] = u.get("losses",0)+1
            bot.send_message(m.chat.id, f"💀 Проигрыш. -{amount} фишек\n💰 Баланс: {get_balance(uid)}", reply_markup=main_keyboard())
        save_data()
    elif game in ("dice","die"):
        # simple dice with bet: win if roll >=4 (example)
        change_balance(uid, -amount)
        roll = random.randint(1,6)
        u = get_user(uid)
        u["games_played"] = u.get("games_played",0)+1
...
if __name__ == "__main__":
    load_data()
    print("🎰 Casino Ruta 5.0 запущен и готов к игре!")
